...
<br>
<footer class="text-muted"> <div class="container"> <p class="float-right"> <a href="#">Kembali Ke Atas</a> </p> ${UI.credit ? '<p>Di desain dengan <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> di unggah ulang oleh <a href="https://t.me/nanons" target="_blank">Aku</a>, Berdasar Sumber Terbuka.</p>' : ''} <p>© ${UI.copyright_year} - <a href=" ${UI.company_link}" target="_blank"> ${UI.company_name}</a>, Seluruh hak cipta untuk pembuat.</p> </div> </footer>
  `;
	$('body').html(html);
}
...
function nav(path) {
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
	...
	html += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Path Saat Ini</a><div class="dropdown-menu" aria-labelledby="navbarDropdown"><a class="dropdown-item"  href="/${cur}:/ ">> Utama</a>`;
    ...
    <a class="nav-link" href="${UI.contact_link}" target="_blank">Hubungi Saya</a>
  </li>`;
...
<form class="form-inline my-2 my-lg-0" method="get" action="/${cur}:Cari">
<input class="form-control mr-sm-2" name="q" type="Cari" placeholder="Cari" aria-label="Cari" value="${search_text}" required>
<button class="btn ${UI.dark_mode ? 'btn-secondary' : 'btn-outline-success'} my-2 my-sm-0" onclick="if($('#search_bar').hasClass('mdui-textfield-expanded') && $('#search_bar_form>input').val()) $('#search_bar_form').submit();" type="submit">Cari</button>
</form>
</div>
</nav>
`;
