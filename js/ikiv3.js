  ...
  var search_bar = `<div class="mdui-toolbar-spacer"></div>
        <div id="search_bar" class="mdui-textfield mdui-textfield-expandable mdui-float-right ${
          model.is_search_page ? "mdui-textfield-expanded" : ""
            ...
            <input class="mdui-textfield-input" type="text" name="q" placeholder="Ketikan yang mau di cari" value="${search_text}"/>
            </form>
...
function onSearchResultItemClick(a_ele) {
  ...
  $.post(`/${cur}:id2path`, { id: a_ele.id }, function (data) {
    ...
    dialog.close();
    dialog = mdui.dialog({
      title: '<i class="mdui-icon material-icons">&#xe811;</i>Gagal mendapatkan link, coba aja lagi',
      content: "Failed to target path, please try again!",
      history: !1,
      modal: !0,
      closeOnEsc: !0,
      buttons: [{ text: "Astoge ???" }],
...
function file_video(path) {
  ...
  const playBtn = `
      <button class="mdui-btn mdui-ripple mdui-color-theme-accent" mdui-menu="{target:'#player-items'}">
        <i class="mdui-icon material-icons">&#xe039;</i>Play di External Player<i class="mdui-icon material-icons">&#xe5cf;</i>
      </button>
